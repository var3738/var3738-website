...
interface LifecycleBentoProps {
  isModal?: boolean;
}

export default function LifecycleBento({ isModal = false }: LifecycleBentoProps) {
  const content = (
    <div className="max-w-7xl mx-auto relative z-10">
      <div className={`mb-20 ${isModal ? 'text-center' : 'text-center lg:text-left'}`}>
         <h2 className={`${isModal ? 'text-4xl' : 'text-4xl lg:text-6xl'} mb-6 font-black tracking-tighter`}>
           The 3-Stage <br />
           <span className="text-primary italic">Lifecycle</span>
         </h2>
         <p className="text-white/40 max-w-xl text-lg font-medium mx-auto lg:mx-0">
      ...
      <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
        {STAGES.map((stage, idx) => (
          <motion.div
            ...
            viewport={{ once: true }}
            className={`modern-card group p-10 flex flex-col justify-between ${isModal ? 'h-[400px]' : 'h-[450px]'} relative overflow-hidden`}
          >
